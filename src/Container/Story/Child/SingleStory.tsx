...
const SingleStory = (props: ISingleStory): JSX.Element => {
  ...
  return (
    <>
      <div className="single-story" onClick={handleStoryDetailpush}>
        <i className="fas fa-book-reader"></i>
        <div style={{ flexBasis: '95%' }}>
          <h1 data-test="SingleStory" className="story-title">
            {props.story.title}
          </h1>
          <p className="story-time">
            {moment(props.story.createdAt).format('ll')}
          </p>
        </div>
        <div className="story-action-wrapper">
          <i
            ...
            onClick={(e: React.SyntheticEvent) =>
              handleUpdateStory(e, props.story)
            }
          ></i>
          <i
            className="fas fa-trash-alt"
            onClick={(e: React.SyntheticEvent) =>
              handleDeleteStory(e, deleteStory, props.story.id)
            }
          ></i>
        </div>
      </div>
